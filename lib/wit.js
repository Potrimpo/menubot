...
const DEFAULT_API_VERSION = '20160516',
  ...
  learnMore = 'Learn more at https://wit.ai/docs/quickstart';
...
function Wit(opts) {
  ...
  const continueRunActions = (sessionId, message, prevContext, i) => {
    return (json) => {
      ...
      const request = {
        sessionId,
        fbPageId: sessions[sessionId].fbPageId,
        fbUserId: sessions[sessionId].fbUserId,
...
const validateActions = (logger, actions) => {
  ...
  Object.keys(actions).forEach(key => {
    ...
    if (key === 'send' && actions[key].length !== 2) {
