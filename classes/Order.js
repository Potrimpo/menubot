...
class Order {
  ...
  get confirmationMsg () {
    ...
    switch (this.depth) {
      case "item":
        responses.push(`Order for one ${this.itemVals.item} @ ${this.readableTime}`);
        break;

      case "type":
        responses.push(`Order for one ${this.typeVals.type} ${this.itemVals.item} @ ${this.readableTime}`);
        break;

      case "size":
        responses.push(`Order for one ${this.sizeVals.size} ${this.typeVals.type} ${this.itemVals.item} @ ${this.readableTime}`);
        break;
    }
    return responses;
  }
